// Parallel matrix multiplication using CUDA - Two Hidden Layers
...
#define HIDDEN_SIZE_2 256 // Number of neurons in the second hidden layer
...
// Function to perform backpropagation and update weights for two hidden layers
void backpropagation(Matrix *X_batch, Matrix *Y_batch, Matrix *Z1, Matrix *Z2,
                     Matrix *Y_pred, Matrix *W1, Matrix *W2, Matrix *W3,
                     int batch_size) {
  // Compute dZ3 = Y_pred - Y_batch
  Matrix *dZ3 = mat_sub(Y_pred, Y_batch);
  mat_scalar_mult(dZ3, 2.0f / batch_size);
...
int main(int argc, char *argv[]) {
  ...
  for (int run = 0; run < NUM_TEST_RUNS; run++) {
    // Allocate and initialize weights for two hidden layers
    Matrix *W1 = allocate_matrix(INPUT_SIZE, HIDDEN_SIZE_1);
    Matrix *W2 = allocate_matrix(HIDDEN_SIZE_1, HIDDEN_SIZE_2);
    Matrix *W3 = allocate_matrix(HIDDEN_SIZE_2, OUTPUT_SIZE);
    random_init(W1);
    random_init(W2);
    random_init(W3);
