...
static GlobalGPUContext global_gpu_context = {0};
...
static void ensure_tile_capacity(int left_column_count,
  ...
  if (needs_realloc) {
    for (int stream_index = 0; stream_index < STREAM_COUNT; stream_index++) {
      ...
      check_cuda(
          cudaMalloc(
              (void **)&global_gpu_context.device_left_tiles[stream_index],
              global_gpu_context.left_tile_byte_count),
          "cudaMalloc(left_tile)");
      check_cuda(
          cudaMalloc(
              (void **)&global_gpu_context.device_result_tiles[stream_index],
              global_gpu_context.result_tile_byte_count),
...
static void initialize_global_gpu_context() {
  ...
  for (int stream_index = 0; stream_index < STREAM_COUNT; stream_index++) {
    ...
    check_cuda(cudaStreamCreate(&global_gpu_context.streams[stream_index]),
               "cudaStreamCreate");
  }
...
static void cleanup_global_gpu_context() {
  ...
  for (int stream_index = 0; stream_index < STREAM_COUNT; stream_index++) {
    cudaFree(global_gpu_context.device_left_tiles[stream_index]);
    cudaFree(global_gpu_context.device_result_tiles[stream_index]);
    check_cuda(cudaStreamDestroy(global_gpu_context.streams[stream_index]),
               "cudaStreamDestroy");
  }
...
static Matrix *matrix_multiply(const Matrix *left_matrix,
  ...
  for (int row_start_index = 0, tile_index = 0;
    ...
    check_cuda(
        cudaMemcpyAsync(result_tile_host,
                        global_gpu_context.device_result_tiles[stream_index],
  ...
  for (int stream_index = 0; stream_index < STREAM_COUNT; stream_index++) {
    check_cuda(cudaStreamSynchronize(global_gpu_context.streams[stream_index]),
